...
/obj/machinery/coffeemaker/RefreshParts()
	...
	for(var/obj/item/stock_parts/micro_laser/laser in component_parts)
		if(laser == /obj/item/stock_parts/micro_laser)
			speed += 1
			continue
		if(laser == /obj/item/stock_parts/micro_laser/high)
			speed += 2
			continue
		if(laser == /obj/item/stock_parts/micro_laser/ultra)
			speed += 3
			continue
		if(laser == /obj/item/stock_parts/micro_laser/quadultra)
			speed += 4
			continue
...
/obj/item/storage/fancy/coffee_cart_rack
	...
	//open_status = FANCY_CONTAINER_ALWAYS_OPEN  //нахуй оно нужно, в душе не чаю.
...
/obj/machinery/coffeemaker/impressa/attackby(obj/item/attack_item, mob/living/user, params)
	...
	if (istype(attack_item, /obj/item/reagent_containers/glass/coffee) && !(attack_item.item_flags & ABSTRACT) && attack_item.is_open_container())
		var/obj/item/reagent_containers/glass/coffee/new_cup = attack_item //different type of cup
		if(new_cup.reagents.total_volume > 0 )
			balloon_alert(user, "the cup must be empty!")
			return
		if(coffee_cups >= max_coffee_cups)
...
/obj/machinery/coffeemaker/impressa/take_cup(mob/user)
	...
	var/obj/item/reagent_containers/glass/coffee/no_lid/new_cup = new(get_turf(src))
	user.put_in_hands(new_cup)
	coffee_cups--
	update_appearance()
