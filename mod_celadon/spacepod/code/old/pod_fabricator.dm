#define PODFAB (1<<9) //Spesspod Fabricator
// #define POD_FABRICATOR (1<<10) //Prints nanite disks
...
// /datum/techweb/specialized/autounlocking/podfabricator
// 	design_autounlock_buildtypes = POD_FABRICATOR
...
/obj/machinery/spod_part_fabricator
	icon = 'mod_celadon/_storge_icons/icons/machine/pod_fabricator.dmi'
	icon_state = "fab-idle"
...
/obj/machinery/spod_part_fabricator/Initialize(mapload)
	stored_research = new
	matching_designs = list()
	rmat = AddComponent(/datum/component/remote_materials, "podfab", mapload && link_on_init)
...
/obj/machinery/spod_part_fabricator/New()
	...
	stored_research = new /datum/techweb(src) //Setup the research data holder.
...
/obj/machinery/spod_part_fabricator/proc/build_part(datum/design/D)
	...
	I.loc = exit
	I.custom_materials[MAT_METAL] = get_resource_cost_w_coeff(D,MAT_METAL)
	I.custom_materials[MAT_GLASS] = get_resource_cost_w_coeff(D,MAT_GLASS)
...
// 		return
// 	var/output
// 	for(var/datum/tech/T in stored_research.researched_designs)
// 		if(T && T.level > 1)
// 			var/diff
// 			switch(T.id)
// 				if("materials")
...
// 		if(!RDC.sync)
// 			continue
// 		for(var/datum/tech/T in RDC.stored_research.researched_designs)
// 			stored_research.AddTech2Known(T)
// 		for(var/v in RDC.stored_research.researched_designs)
// 			var/datum/design/D = SSresearch.techweb_design_by_id(v)
// 			stored_research.AddDesign2Known(D)
// 		stored_research.RefreshResearch()
...
/obj/machinery/spod_part_fabricator/proc/AfterMaterialInsert(item_inserted, id_inserted, amount_inserted)
	var/datum/material/M = id_inserted
	add_overlay("fab-load-[M.name]")
	addtimer(CALLBACK(src, TYPE_PROC_REF(/atom, cut_overlay), "fab-load-[M.name]"), 10)
	updateUsrDialog()

/obj/machinery/spod_part_fabricator/proc/search(string)
...
/obj/machinery/spod_part_fabricator/proc/do_process_queue()
...
/obj/machinery/spod_part_fabricator/maint
	link_on_init = FALSE
